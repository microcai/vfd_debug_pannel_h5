...
0x02 | operation_mode | 8 | 读写 |控制模式。0:停机, 1:频率模式, 2:电压模式, 3:扭矩模式, 4:速度模式, 5:位置模式, 6:快速位置模式, 7:手动V/F模式
0x03 | direct_control_command | 8 | 读写 |直接控制命令。如：重读EEPROM配置，写入 EEPROM 配置
0x04 | VBUS | 32 | 只读 |32位浮点数。单位 V
0x08 | OutputPower | 32 | 只读 |32位浮点数，单位 W
0x0C | Iq | 32 | 只读 |32位浮点数，单位 A
0x10 | Id | 32 | 只读 |32位浮点数，单位 A
0x14 | cur_freq | 32 | 只读 |32位浮点数，单位 Hz
0x18 | cur_torque | 32 | 只读 |32位浮点数
0x1C | cur_speed | 32 | 只读 |32位浮点数，单位 Hz
0x20 | cur_position | 32 | 只读 |32位浮点数，单位 度
0x24 | target_freq | 32 | 读写 |32位浮点数，单位 Hz
0x28 | target_torque | 32 | 读写 |32位浮点数（电压或扭矩命令）
0x2C | target_speed | 32 | 读写 |32位浮点数，单位 Hz
0x30 | target_position | 32 | 读写 |32位浮点数，单位 度
